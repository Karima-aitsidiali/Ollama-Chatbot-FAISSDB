...
class ResourceManager:
    ...
    def get_filiere_by_departement(self, dep_id: int) -> Optional[dict]:
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM filieres WHERE departement_id = ?", (dep_id,))
            return [dict(row) for row in cursor.fetchall()]
    ...
    def get_module_by_filiere(self, filiere_id: int) -> Optional[dict]:
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM modules WHERE filiere_id = ?", (filiere_id,))
